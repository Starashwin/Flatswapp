...
class PropertyFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = Property
        fields = ['multi_name_fields','postcode','n_bedrooms','furnished','price','date']
         #groups = CombinedGroup(filters=['name', 'postcode'], combine=operator.or_)
    
    def filter_by_all_name_fields(self, queryset, name, value):
        return queryset.filter(Q(name__icontains=value) | Q(postcode__icontains=value) | Q(outward__icontains=value) | Q(description__icontains=value) | Q(nearest__icontains=value)| Q(neighbour__icontains=value))
